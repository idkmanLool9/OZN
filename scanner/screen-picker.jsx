// Screen — Dossier kiezen waarin gescande gegevens belanden.
// Toont een lijst van bestaande dossiers uit dezelfde Supabase-database
// als de uitvaart-app, met zoekfilter op naam/nummer.

function PickerScreen({ onPick, onSignOut }) {
  const [dossiers, setDossiers] = React.useState(null); // null = loading
  ...
  const filtered = React.useMemo(() => {
    ...
    return dossiers.filter(d => {
      const blob = [
        d.dossier_nummer, d.voornaam, d.achternaam,
        d.contact_naam, d.contact_voornaam,
  ...
  return (
    <ScreenShell>
      ...
      <div style={{
        ...
        <TertiaryButton onClick={onSignOut} style={{ width: 'auto', height: 32, padding: '0 12px' }}>
          Uitloggen
        </TertiaryButton>
      </div>
      ...
      <div style={{ padding: '0 24px 8px' }}>
        ...
        <div style={{ fontSize: 14, color: T.muted, marginTop: 6, lineHeight: 1.45, letterSpacing: -0.05 }}>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              fontFamily: T.font, fontSize: 15, color: T.ink, height: '100%',
            }}
          />
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 20px' }}>
        ...
        {filtered && filtered.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtered.map(d => (
              <DossierRow key={d.id} d={d} onClick={() => onPick(d)}/>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        <Icon.Person size={20} c={T.blue}/>
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: T.ink, letterSpacing: -0.2, lineHeight: 1.2 }}>
          {fullName}
        </div>
        <div style={{ fontSize: 12, color: T.muted, marginTop: 2, letterSpacing: -0.05, lineHeight: 1.35,
