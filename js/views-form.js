...
// R2-map-context bij een upload: leest live uit het huidige intake-formulier
// zodat de bestanden op R2 onder de juiste dossier-map belanden. Voor bestaande
// dossiers (met id) neemt 'ie ook het id mee zodat de map uniek is bij naamgenoten.
function _dossierCtxForUpload() {
  try {
    const $f = (n) => document.querySelector(`#dossier-form [name="${n}"]`);
    const achternaam    = ($f('achternaam')?.value    || '').trim();
    const voornaam      = ($f('voornaam')?.value      || '').trim();
    const dossier_nummer= ($f('dossier_nummer')?.value|| '').trim();
    // dossier.id staat niet in de form (autogenerated na save), maar het pad
    // (bv. #/dossiers/42/wizard) geeft 'm wel prijs.
    const m = (location.hash || '').match(/#\/dossiers\/(\d+)/);
    const id = m ? parseInt(m[1], 10) : null;
    return { achternaam, voornaam, dossier_nummer, id };
  } catch (_) { return null; }
}
...
function renderDossierForm(params) {
  ...
  (function initBezitFotos() {
    ...
    async function upload(inp, tag) {
      ...
      try {
        return await BezittingenFotos.upload(file, tag, _dossierCtxForUpload());
      } catch (_) { return null; }
    }
  ...
  async function doArtsUpload(file) {
    ...
    try {
      const path = await ArtsVerklaring.upload(file, 'artsverklaring/', _dossierCtxForUpload());
  ...
  async function doOverdraagUpload(file) {
    ...
    try {
      const path = await ArtsVerklaring.upload(file, 'overdraagformulier/', _dossierCtxForUpload());
